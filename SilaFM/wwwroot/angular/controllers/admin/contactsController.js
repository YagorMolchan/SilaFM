...
app.controller('contactsController', function ($scope, $http, $compile, $window) {
    $scope.model = model;

    $scope.addItem = function (e, array) {
        e.preventDefault();
        if (!array)
            array = [];
        array.push({});
    ...
    $scope.addSocial = function (e) {
        e.preventDefault();
        if (!$scope.model.Contacts.SocialsList)
            $scope.model.Contacts.SocialsList = [];
        $scope.model.Contacts.SocialsList.push({ Type: 'Facebook' });
    };

    $scope.deleteItem = function(e, array, item) {
        e.preventDefault();
        if (array && item)
    ...
    $scope.removeSocials = function (e, item) {
        e.preventDefault();
        var index = $scope.model.Contacts.SocialsList.indexOf(item);
        if (index >= 0) {
            $scope.model.Contacts.SocialsList.splice(index, 1);
        }
    };
